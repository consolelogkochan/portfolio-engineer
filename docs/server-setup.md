...
【解消の予定】デプロイをCIビルド＋成果物転送に切り替えた時点で（7-4e）、本番サーバーからNode.jsを削除する。その時点でこの不一致は解消される。
...
サーバー上で手作業の `git pull` をしないこと。素の `git pull` はシェルの umask（0002）で動くため、取得したファイルにグループ書き込みが付く（通常ファイルは `0666 & ~0002` = 664、実行可能ファイルとディレクトリは `0777 & ~0002` = 775）。`bin/deploy.sh` は自分の中で umask 027 を設定するが、既にあるファイルの権限は変えないため、後から実行しても直らない。

取得は必ず `./bin/deploy.sh` を通す。やむを得ず手で引く場合は `(umask 027 && git pull --ff-only)` とする。

実測（2026-09-05）：`bin/deploy.sh` をサーバーへ初めて取得したとき、この形で引いたため 750（`-rwxr-x---`）になった。
...
- 通常：GitHub の Actions で `Deploy` を手動起動する。ワークフローは CI 専用の鍵で ssh を1回打つだけで、サーバー側の `bin/deploy.sh` が動く。ワークフローが送る文字列は `authorized_keys` の `command=` により無視される。
- 緊急：GitHub に到達できないときや revert を待てないときは、従来どおり人がサーバーで `bin/deploy.sh` を実行する（コマンド列は `bin/deploy.sh` を参照）。この節の「失敗したときの巻き戻し」の2経路はそのまま有効。
...
`deploy.yml` は同時に2つ走らないようにしてある（`concurrency`）。`bin/deploy.sh` の `npm ci` は `node_modules` を削除してから入れ直すため、2つのデプロイが重なると、片方が削除している最中にもう片方がビルドを読みに行って途中で落ち、`npm run build` の出力先（`public/build/`）が半端な状態で残りうる。これは本番が壊れた状態そのものである。走っている途中のデプロイは止めず、後から来たものを待たせる設定にしている（途中で止めるほうが危険なため）。`public/build/` は Git の管理外（`.gitignore` の対象）なので、`bin/deploy.sh` の作業ツリーのクリーン確認ではこの破損を検出できない。
...
`command=` が固定するのは**実行するファイルのパス**であって、そのファイルの中身ではない。`bin/deploy.sh` はリポジトリ内のファイルで、最初に行うのは `git pull` である。

- `main` に `bin/deploy.sh` を書き換えたコミットが入れば、次のデプロイでそれが本番で走る。
- `bin/deploy.sh` を書き換えなくても、`composer.json` の `scripts`、npm パッケージのインストールスクリプト、ビルド設定ファイルは、いずれもリポジトリの中身で決まる。
...
- **7-4eでNode.jsをサーバーから削除すれば、universe依存の大半が消える**
