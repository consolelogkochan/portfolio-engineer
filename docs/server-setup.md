...
### 41-1. 7-4a（手順書の監査）の残りで回収する

**7-4 の最初のカードは、この文書そのものの監査とする。** 7-4a-1（根拠の監査）は完了。以下は 7-4a-2（依存と参照の監査）以降で扱う、手順書の書き方・検証状態に関する項目である。

| 項目 | 補足 |
| --- | --- |
| 手順書の実行順序が通しで検証されていない | Issue #27 |
| 節番号での相互参照を、名前での参照に変えるべきか | 節を追加するたびに参照の修正が要る。7-3d でも1度、依頼文で誤った節番号を書いた |
| バックアップの破棄時期が手順書に規定されていない | 7-3d-2a では「取得完了時に削除」と事前に決めて実行したが、規定としては書かれていない |
| pollinate が不要パッケージとして残っているか未確認 | 7-3d-1 で `apt autoremove` の実施を「7-3d-1 の区切り」に持ち越したが、**実施したかどうかが記録されていない。** 監査の中で状態を確認する |

---

### 41-2. 7-4（デプロイの自動化）で回収する
...
推測（未検証）：したがって「`umask 027` だけで正しい権限になる」は、そのままでは成り立たない。`www-data` による書き込みを要するパス（`storage` など）には、別の umask か個別の権限付与が要る。どのパスにグループ書き込みが必要かは、29節・30節の現状と突き合わせて 7-4 で確定する。
...
**`opcache.validate_timestamps = 0` の再検討**

デプロイが自動化されるまで保留。自動化されれば、デプロイ後に opcache を確実にリセットできるため、無効化の前提が整う。
