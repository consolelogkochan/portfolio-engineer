...
Node.js の実際の削除（apt パッケージの除去）を行った。新方式（7-4e）が本番で成功したことを確認したうえで、2段階に分けて進めた。取り消しにくい操作のため、各段階の実行前に `--dry-run` で削除対象を一覧確認した。実測（2026-09-11）：
...
| 入れると継続的な負担が生まれる（新しい機能APIが追加されても既存のヘッダは拒否せず、カバー範囲が黙って減っていく） | 根拠に書かれた情報からは、この負担が解消される条件を導けない |
