...
npm はカレントディレクトリからプロジェクトの `.npmrc` を探して適用する。`-g`（グローバルインストール）でも同じで、リポジトリの中から実行した `npm install -g` にも効く。このため、開発機での作業・CI の `npm ci`（`ci.yml`）・本番サーバーの `bin/deploy.sh` の `npm ci` の3か所すべてに適用されている。実際、リポジトリ内から実行した `npm install -g` が postinstall を実行できず、開発機で Claude Code が起動しなくなる形で表面化した（7-4d の作業中）。
