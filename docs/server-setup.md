...
本番サーバーを構築したときの手順と、**その判断の根拠**を記録したものです。

サーバーを作り直すときの出発点として使えますが、**上から順に実行すれば同じ状態に到達することは検証していません。** 通しでの実行検証は行わないと決めました（Issue #27）。理由は3つです。

- 検証は、実行した時点の記録にしかならない。21節が、実測はその時点の記録にすぎず将来を保証しないと述べているが、通し検証もその例外ではない
- 証明書に関わる節（33〜39節）は、名前が1つしか無いため複製して試せない。「全体を通しで検証した」という状態は原理的に作れない
- 部分的にしか成立しない「検証済み」は、備えがあると思い込ませる分だけ危険である

したがって、この文書の価値は判断の根拠にあります。何をしたかより、**なぜそうしたか、何を差し出して何を得たか**が書いてあります。作り直すときは、同じ判断が今も成り立つかを確かめながら読んでください。

## そのまま流してはいけない箇所

次の節は、時間が経つだけで内容が古くなります。

| 節 | 古くなる理由 |
|---|---|
| 13・18 | PHP・Composer のバージョン。導入した時点のものが書いてある |
| 21 | Node のバージョン。CI（`ci.yml`・`deploy.yml`）が指定する `node-version` と、`package.json` の `engines` が書いてある。サーバーには Node が無い（7-4e） |
| 16 | Ubuntu が配布する php.ini の既定値。版によって変わる。7-4f で加えた opcache・`pm.max_requests` などの実測値も、測定した時点のものである |
| 35・37 | certbot は snap で自動更新される。証明書の有効期間も短縮が予定されている |
| 37 | 残り日数の検知の閾値は、現在の有効期間を前提にしている |
| 1・7 | ConoHa のセキュリティグループと、cloud-init が配置するファイル |
| 34 | ムームードメインの管理画面の構造 |
...
他の節を指すときは、その節の字句を引用せず、地の文で要約して**節番号だけを指します**。字句は表記ゆれと書き換えで照合が不安定になりますが、節番号は機械的に照合できるためです。

### バックアップの扱い

設定ファイルを変更する前に取ったバックアップ（`.bak`）は、**変更の検証が通った時点で削除します**。残す目的は戻せることですが、検証が通れば戻す理由が消えます。残したまま次の変更を加えると、どの時点のものか分からなくなり、元のファイルが更新されても追随しません。

配布元のファイルを無効化したもの（7節の `100-allowsshrsa.conf.disabled` など）は、自分が変更したファイルの控えではないため、対象外です。

### 節どうしの関係

ある節が別の節を指すとき、関係は3種類あります。軸は「**その節はいつ必要か**」です。

| 種類 | いつ必要か | 上から流すと |
|---|---|---|
| 言及 | 不要。判断や記述に触れているだけ | 何も起きない |
| 前提 | その節より**前**に | その節で止まる |
| 維持依存 | その節の**後、継続的に** | 止まらない。達成した状態が後から黙って失われる |

維持依存の一覧は40節にあります。

### 実測に日付を添える

測定・観測・調査の結果を書くときは、実測した日付を添えます。値は時間で古くなりますが、日付があれば「いつの時点のものか」が分かり、読む人が自分で判断できます。

設定値（そう設定したという値）は対象外です。設定値は時点ではなく仕様を表すため、日付が無くても古くなりません。

この約束より前に書かれた記録には遡及しません。後から日付を埋めれば推測になり、記録の質がかえって下がるためです。
...
【7-4d】この節は `sshd_config` 全体に効く設定である。7-4d で CI デプロイ用の鍵を1本追加し、その鍵に限っては `authorized_keys` 側の `command=` と `restrict` で実行できる操作を固定した。鍵ごとの制限の内容と理由は 30節に記録している。
...
- **メール送信のAPIキー。** `bootstrap/cache/config.php`にAPIキーが平文で入っており、www-dataが読める（実測 2026-09-23：このファイルは640でグループがwww-data）。**www-dataを取られた場合、このドメインの名前でDKIM署名付きのメールを送れる。** キーは差し替えられるが、**送られたメールは回収できない。** この露出は設定の誤りではなく構造である——秘密情報を使う主体が読めなければ使えないため、`storage/`について同じ理由で残ると述べたのと同じ形である。緩和は、キーの権限を送信のみに絞り、送信元ドメインを1つに限定することである（19節）。**ただし実測（2026-09-23）：全権限のキーには送信元ドメインを限定する仕組みが無い。同じアカウントに全権限のキーが1本でもあると、この緩和は効かない**（7-10）
...
OPcacheが実際に稼働していることは、14節でphpinfo()により確認済みである（Zend OPcacheのセクションが存在し、Opcode Cachingが`Up and Running`）。
...
- デプロイを自動化してreloadを手順に組み込めば「忘れる」経路が消えるため、その時点（7-4f）で`0`を再検討する
...
【7-4f】この実測を行った。下記「再検討（7-4f）」を参照。

### 再検討（7-4f）：`opcache.validate_timestamps` は変えない

7-4e でデプロイが自動化され、上の判断が再検討の条件としていた「reload を手順に組み込める」状態が整ったため、本番サーバーで実測して再検討した。**結果：変えない（既定の`1`のまま）。**

**測定方法**（将来また測るときのために記録する）

- 一時的な PHP ファイルを `public/` に置き、`ini_get('opcache.validate_timestamps')` の値を返させて、PHP-FPM が実際に使っている値を確認した（設定ファイルの記載ではなく実効値を見るため）。
- `curl -u <USER> -s -o /dev/null -w '%{time_total}\n' 'https://new.ikshowcase.site/?n=[1-40]'` でトップページを40回引き、後半20回の平均で比較した。
- 設定の切り替えは `/etc/php/8.5/fpm/conf.d/` に `99-*.ini` を置いて `systemctl reload php8.5-fpm` で行った（副産物：この置き場所で FPM の opcache 設定を変えられることを実測で確認した。次に再検討するとき置き場所を探さずに済む）。
- 測定後、一時ファイルと実験用の `.ini` は削除した。

**実測（2026-09-12）**

| 測定 | 設定 | ワーカーの状態 | 後半20回の平均 |
| --- | --- | --- | --- |
| 1 | `=1` | 長く生きたワーカー | 28.9 ms |
| 2 | `=1` | 長く生きたワーカー | 31.6 ms |
| 3 | `=0` | reload 直後 | 20.6 ms |
| 4 | `=1` | reload 直後 | 22.2 ms |

**読み取り**

- 設定そのものの効果（測定3と4。条件が揃っており違いは設定だけ）は1.6 ms。各回の値は15〜30 msに散らばっており、20回平均の不確かさはおよそ±1 ms。**1.6 msの差はこのばらつきと同じ大きさで、この測定では区別できない。**
- ワーカーの状態による差（測定1・2 対 測定3・4）は約30%。方向は一貫しているが、各条件の観測が2回だけで、しかも古い2回が先・新しい2回が後という時間の並びになっている。外的な要因と区別できていないため、**示唆的だが確定していない。**

**判断（変わらず）**：`opcache.validate_timestamps` は既定の `1` のまま。

- 効果が測定のばらつきに埋もれる。
- 代償は明確に大きい。`0` にすると、デプロイのたびに opcache を明示的にリセットする必要が生じる。素直な方法は php-fpm の reload で、これには管理者権限が要る。**`bin/deploy.sh` に `sudo` が戻ることになる**（30節で `sudo` を使わない設計にしたスクリプト）。避けるにはアプリ側にリセットの経路を作る必要があり、その設計と認証が別途要る。
- `0` にすると「デプロイのたびに reload が必須」という維持依存が1本増える。

**再検討すべき条件**

| 条件 | 何が変わるか |
| --- | --- |
| 応答時間が課題になったとき（トラフィックの増加、ページの重量化など） | 効果の側。現状は20〜30 msで体感に影響しないが、課題になれば数%でも取りに行く価値が出る |
| `sudo` を使わずに opcache をリセットできるようになったとき | 代償の側。代償が消えれば、効果が小さくても採れる |
| 測定の精度を上げられるとき | 根拠の側。今回は測定ごとのばらつきが効果より大きかった。負荷をかけた状態で条件を交互に切り替えて多数回測れば、効果の有無を確定できる |

### 実測（7-4f）：opcache のサイズ設定は十分

`opcache_get_status()` で確認した（2026-09-12）。サイズ設定（`memory_consumption` / `max_accelerated_files`）は変えない。

| 項目 | 値 |
| --- | --- |
| メモリ | 22.2 MB / 128 MB 使用（`memory_consumption` は既定の128） |
| キャッシュ済みスクリプト | 476 / 10000（`max_accelerated_files` は既定） |
| `wasted_memory` | 0.0 MB |
| `oom_restarts` / `hash_restarts` | 0 / 0 |
| hits / misses | 18135 / 477（ヒット率97.4%。missesはほぼ初回コンパイル分） |
...
### 判断：`pm.max_requests = 500` を設定する（7-4f）

`/etc/php/8.5/fpm/pool.d/www.conf` の該当行（設定ファイル自身が例として示していた値）のコメントを外し、`pm.max_requests = 500` にした。処理したリクエスト数がこの値に達したワーカーは、次のリクエストの後に作り直される。

設定前は未設定（既定値 `0` ＝無制限）で、php-fpm を起動してから一度もワーカーが作り直されていなかった。

**確認**

- `sudo php-fpm8.5 -t` → `test is successful`
- `sudo systemctl reload php8.5-fpm` → `active`
- `sudo php-fpm8.5 -tt` → `NOTICE: pm.max_requests = 500`（実効値として確認）

**判断の理由**

- 上の「再検討（7-4f）：`opcache.validate_timestamps` は変えない」の測定で、「長く生きたワーカーの方が遅い」という観測があった。**ただし原因は特定できておらず、再現も確認していない。**
- 代償がほぼゼロ。設定1行で、opcache は共有メモリ上にありワーカーを作り直しても消えないため、デプロイ手順も変わらず、維持依存も増えない。

**証拠の強さの使い分け**：上の `opcache.validate_timestamps` は代償が明確に大きいため、効果を確かめてから採る方針にした（不確かな効果では採らなかった）。この `pm.max_requests` は代償がほぼゼロなので、効果がまだ不確かな観測（ワーカー劣化の約30%差）だけを根拠に採用した。**代償の大きさに応じて、要求する証拠の強さを変えている。**

**限界**：観測された劣化が時間経過によるものなら、リクエスト数で数える `pm.max_requests` では防げない。現在のサイトは訪問がほとんど無く、500 に達するまで相当かかる。**7-9（公開後）に再測定すべき項目。**
...
【7-5への影響】`proc_open` を無効化したため、`MAIL_MAILER=sendmail` は使えない。Resendは SMTP または HTTP API を使うため問題ないが、送信方式を決める際の制約になる。**実測（2026-09-25）：`vendor/symfony/mailer`の`sendmail`のトランスポートが、外部プロセスを起動して`proc_open`を呼んでいることを、コードを読んで確認した。**

**【追記（7-5b）】メール送信に使う関数が無効化されていないことを確認した。**

実測（2026-09-25）：`php-fpm8.5 -i`が出力する実効値は`exec,passthru,shell_exec,system,proc_open,popen,pcntl_exec,dl`の8件で、ローカル値とマスター値は一致していた。同日、`/etc/php/8.5/fpm/pool.d/`にこの設定を上書きする記述が無いことも確認した。**SMTPでの送信が使う`stream_socket_client`・`stream_socket_enable_crypto`・`fsockopen`および読み書きの関数は、いずれも無効化されていない。**

**この確認はコマンドの出力によるものであり、FPM経由で実際に1通送った結果ではない。** 実際の送信による確認は7-5cで行う。
...
**【追記（7-5b）】メール送信の設定について**

**送信方式はSMTPにした。** 理由は4つである。

1. **13節の方針。** 入れるソフトが減れば、攻撃面も更新の対象も減る。HTTP APIを使うにはパッケージが1つ必要になり、**本番で実行される他人のコードが1つ増え、`composer audit`の対象も1つ増える**
2. **SMTPなら`MAIL_SCHEME`・`MAIL_HOST`・`MAIL_PORT`の3つが意味を持つ。** HTTP APIを選ぶと、この3つは使わない設定として残る
3. **SMTPは共通の規格であるため、送信サービスを乗り換えるときも設定の変更だけで済む。** HTTP APIはサービスごとに違う
4. **設定の段階数が2対6である。** HTTP APIはデプロイを伴う

**`MAIL_SCHEME`を明示する理由。** 実測（2026-09-23）：この値が空欄の場合、Laravelはポートが465なら暗号化する方式を、それ以外なら平文で始まる方式を自動的に選ぶ。**自動で決まるということは、効いている値が`.env`から読めないということである。** この節の方針に従って明示する。

**APIキーは、権限を送信のみに絞り、送信元ドメインを1つに限定したものを使う。** この判断の理由と、**その限界**（全権限のキーが1本でもあると、この限定が効かないこと）は、15節に記録した。

**`SESSION_COOKIE`を明示する理由。** 実測（2026-09-23）：`config/session.php`は、この値が未設定のときの既定値として`APP_NAME`を使う。**そのため`APP_NAME`を変えると、セッションCookieの名前が変わり、既存のセッションが無効になる**（推測。確認していない）。**明示すれば、この結びつきが切れる。**

**この節の表と、本番の`.env`の全キーとの関係。** 実測（2026-09-23）：本番の`.env`には50件のキーがある。**この作業を始める前の時点で、この文書が扱っていたのは9件だけだった**（この節の表が7件、20節が1件、38節が1件）。**残る40件には記載が無い。**

**全キーの一覧をこの節に置くことはしない。** `.env.example`が変わるたびに古くなり、**維持依存が1本増えるためである。** この節の方針は、**判断が必要だったキーを記録することであり、キーを網羅することではない。** 記載の無いキーについて判断が必要になった時点で、その都度この表に足す。
...
| APP_NAME | `伊崎孝太郎 ポートフォリオ` | 実測（2026-09-23）：この値が効くのは、メールの差出人の表示名、メール本文の署名、Laravel既定のメールレイアウトのヘッダーとフッター、およびセッションCookieの名前である。**サイトのタイトルには現れない。** 効く場所がすべて自分の受信箱であるため、**受信箱で識別しやすいことだけを基準に決めた** |
| MAIL_MAILER | smtp | 送信方式。下の方針を参照 |
| MAIL_SCHEME | smtps | 接続した直後に暗号化する方式。`MAIL_PORT`と対になる。**空欄でも自動的に決まるが、明示する**（下の方針） |
| MAIL_HOST | smtp.resend.com | Resendの受け口 |
| MAIL_PORT | 465 | **平文でやり取りする段階が存在しないポート。** 587は平文で接続してから暗号化に切り替える |
| MAIL_USERNAME | resend | Resendでは固定の文字列であり、アカウント名ではない |
| MAIL_PASSWORD | 値はこの文書に書かない | ResendのAPIキーそのもの。**権限は送信のみに絞り、送信元ドメインを1つに限定したものを使う**（15節） |
| MAIL_FROM_ADDRESS | `noreply@ikshowcase.site` | 差出人として相手に見えるアドレス。**apexを使う理由は34節** |
| MAIL_FROM_NAME | `${APP_NAME}` | **参照のまま残す。** `APP_NAME`が実際に効く場所がメールであるため、別の値を重ねて書く意味が無い |
| CONTACT_TO | 値はこの文書に書かない | 問い合わせの届け先。**`.env.example`には無く、本番にのみ存在する** |
| SESSION_COOKIE | `ikshowcase_session` | **明示しないと`APP_NAME`から作られる**（下の方針） |
| SPAM_MIN_SECONDS | `3` | 問い合わせフォームを開いてから送信までの秒数がこれ未満のとき、送信せずログに記録する。**推測：人間が4つの項目を埋めて3秒で送り終えることは無いため、正当な利用者を止める恐れは小さい。確認していない。** `config/spam.php` の既定値と同じ値である |
| SPAM_MAX_URLS | `2` | 本文に含まれるURL（`http://`または`https://`で始まるもの）がこれを超えると、**送信せず利用者にエラーを返す。** この値は正当な利用者も止めうるが、**このサイトの主な用途が学習の成果と作品の受け皿であり、問い合わせから案件を受けることを主には想定していないため、止める側の代償が軽いと判断した**（2026-09-26）。**再検討の条件：サイトを仕事の窓口として主に使う方向に変えるとき、または公開後にスパム判定の記録が増えたとき。** `config/spam.php` の既定値と同じ値である |
...
**`--no-dev`を選ぶ理由**：開発用のツール（PHPUnit、Pint、Sailなど）は本番の実行に不要であり、本番に置かない。
...
### サーバーに Node.js を入れない（7-4e）

アセットのビルドは CI（GitHub Actions）で行う。**サーバーに Node.js は入れない。**
...
Node.js の実際の削除（apt パッケージの除去）を行った。新方式（7-4e）が本番で成功したことを確認したうえで、2段階に分けて進めた。取り消しにくい操作のため、各段階の実行前に `--dry-run` で削除対象を一覧確認した。実測（2026-09-10）：

1. `nodejs` と `npm` を `purge` する前に `--dry-run` で確認した：**144個が削除対象**。すべて JavaScript / Node の生態系で、PHP・nginx・SSH・systemd の系統は1つも含まれていなかった。実行後、`which node npm` が何も返さないこと、nginx と php-fpm がともに `active` であること、サイトの表示に問題が無いことを確認した。
2. **Node が無い状態で `Deploy` ワークフローを実行し、緑になることを確認した。** 7-4e の目的（サーバーからビルド用のランタイムを無くす）の達成確認にあたる。
3. 不要になった依存を `autoremove --purge` する前に `--dry-run` で確認した：**さらに316個**。実行後、nginx と php-fpm がともに `active`、`nginx -t` が成功すること、サイトの表示に問題が無いことを確認した。

合計460個（144 + 316）が削除された。32節の保留3件についての確認は、この翌日に別途行った。

316個の内訳には、`node-*`（200個以上）や `libjs-*`、Node が使う C ライブラリに加え、**X11 と描画関連**（`x11-common` `x11-utils` `xdg-utils` `libx*` `mesa-*` `libgl1` `libvulkan1` など）、Perl のユーティリティ、**`alacritty`（ターミナルエミュレータ）**、`gyp`、`wabt`、`nodejs-doc` が含まれていた。**つまり、当初 `apt install npm` の一言で、画面の無いサーバーに X11・グラフィックスドライバ・ターミナルエミュレータまで入っていた。** これは「サーバーにビルド用のランタイムを入れない」という7-4e の判断を裏づける観測である。正確な依存の連鎖は追っていない。`xdg-utils`（URL を既定のアプリで開く仕組み）を経由していると考えられるが、**これは推測であり、確認していない。**

削除前に、`libssl-dev`（開発用ヘッダ）は削除対象に含まれるが、`libssl3`（nginx と PHP が使う本体）は含まれないことを確認したうえで実行した。実行後の nginx / php-fpm の稼働確認は、この点が実際に壊れていないことの裏づけにもなっている。

### 移行前の記録（サーバーでビルドしていた頃）

以下は 7-4e より前、サーバー上で `npm ci` / `npm run build` を実行していた頃の記録である。ビルドを行う場所が CI の1か所になったため、「環境をまたいで成果物が一致する」という要件そのものが無くなった。記録は残す。

7-4e より前の手順は次のとおりだった（`sudo apt install -y nodejs npm` の後）。
...
`npm audit`のゲートは二段にした。1段目は本番に載る依存（`--omit=dev`）を対象にhigh以上で失敗させ、2段目はdevを含む全依存を対象にcriticalで失敗させる。本番に載るかどうかで基準を変えており、開発だけに載る依存には緩い。緩めてよいと判断したのは、`ci.yml`（このワークフロー）がsecretsを一つも使っておらず、ビルド時に悪意あるコードが動いても被害の上限が「CIが落ちる・遅くなる」に留まるためである（実測：`ci.yml`にsecretsの参照は無い）。7-4dで`deploy.yml`にsecretsが入ったが、これは別のワークフローファイルであり、この判断の前提（`ci.yml`にsecretsが無いこと）は変わっていない。
...
```bash
(umask 027 && php artisan config:cache && php artisan route:cache && php artisan view:cache)
php artisan about --only=cache
```

**上のコマンド例が`(umask 027 && …)`の形になっているのは、次の理由による。** 実測（2026-09-23）：この形で実行した直後の`bootstrap/cache/config.php`は640で、グループはwww-dataだった。**7-4bで定めたumask 027はデプロイの手順の中に置かれているため、手作業の実行には効かない。** 38節が記録しているとおり、素のまま実行するとシェルのumask（0002）に従って664になる。**このファイルにはメール送信のAPIキーが入る**ため（15節）、22節が定めたその他に対して閉じる設計のとおりにする。29節が同じ形で実行している。
...
config:cacheを実行した直後のbootstrap/cacheを確認すると、生成されたファイルは`-rw-rw-r--`（664）になっている。22節で設定したo=（その他は権限なし）が効いていない。
...
3つ目の検証（`Host`ヘッダを付けてIPへ直接アクセスする）の意味：DNSを経由せずサーバーを名指しで確かめる手段が残っている。1節でICMPを含めたのと同じ、観測できる手段を自分から捨てないという判断である。
...
- 自分の内容で保持し続けると、`nginx-common`が更新されるたびに上書き確認が出る。5節で、上書き確認が出たら止めて確認すると決めた事象を、自分で作り続けることになる
...
- **worker 1つあたりの実質増分は約20MB。** `max_children = 10`にするとPHP-FPM全体が約220MB、総メモリ1961MBの11%になる見込み（実際の変更は27節）
...
26節の実測にもとづき、`/etc/php/8.5/fpm/pool.d/www.conf`を直接編集した。
...
sudo rm /etc/php/8.5/fpm/pool.d/www.conf.bak
```

`-tt`で実効値が意図した値になっていることを確かめた時点で、`.bak`を戻す理由が消える。冒頭の約束による。
...
**【後日の訂正（7-5b）】この継続採用しないという判断は、7-4b以降は逆になっている。** 7-4bでデプロイ時のumask 027を採用したため、**`(umask 027 && ...)`の形は現在の正しい方式である。** 7-4bの際にこの節へ追記された注記は`chmod`についてのみ触れており、**この継続採用しないという記述には及んでいなかった。** 実測（2026-09-23）：手作業の`config:cache`をこの形で実行し、640になることを確認した（23節）。

**この訂正が扱っているのは、判断が逆になったという事実だけである。上に挙げた2つの理由が7-4b以降も成立するかは、検討していない。**
...
この手順は 7-4b の段階2で `bin/deploy.sh` としてスクリプト化した（その後 7-4d・7-4e で改訂）。現在の内容は以下「手順」以降を参照。

### 手順

デプロイは GitHub の Actions から `Deploy` を手動起動する（詳細はこの節の「GitHub Actions からの起動」「ビルドを CI へ移した」）。`bin/deploy.sh` は CI 専用で、人はサーバーで直接実行しない。CI がビルドした成果物（`public/build/` の中身 + ビルドしたコミットのハッシュ）の tar.gz を標準入力で受け取る。サーバーに Node.js は無く、ビルドはしない（21節）。

`bin/deploy.sh` が行うこと（順に）

1. 標準入力から成果物を受け取り、一時ディレクトリへ展開する
2. 成果物に `build/manifest.json` と `COMMIT` があることを確認する
3. `public/build` が symlink であることを確認する（実体のディレクトリならこの節の移行手順を促して中止）
4. 作業ツリーがクリーンで、`main` ブランチにいることを確認する
...
サーバー上で手作業の git 操作（`git pull` / `git merge` / `git checkout`）をしないこと。`bin/deploy.sh` は自身の中で umask 027 を設定し、`git fetch` と `git merge --ff-only` を行う。素の `git pull` はシェルの umask（0002）で動くため、取得したファイルにグループ書き込みが付く（通常ファイルは `0666 & ~0002` = 664、ディレクトリと実行可能ファイルは `0777 & ~0002` = 775）。ビルド成果物を前の世代に戻すときは `bin/rollback.sh` を使う（「失敗したときの巻き戻し」）。

実測（2026-09-05）：`bin/deploy.sh` をサーバーへ初めて取得したとき、`(umask 027 && git pull --ff-only)` で引いたため 750（`-rwxr-x---`）になった。

### GitHub Actions からの起動（7-4d）

デプロイの起動が、人がサーバーに入って実行する形から、GitHub の Actions 画面で `Deploy` ワークフロー（`.github/workflows/deploy.yml`）を手動起動する形に変わった。

`deploy.yml` は `ci.yml` と書き方を揃えている。ステップに `name:` を付けず、説明は直前の日本語コメントで書く。`ci.yml` の既存のステップに `name:` が1つも無いところへ、新しいワークフローだけ `name:` を付けると不揃いになるため、この方針をそのまま踏襲した。

**起動経路**

- 通常：GitHub の Actions で `Deploy` を手動起動する。`build` ジョブがビルドし、`deploy` ジョブが成果物を tar にして CI 専用の鍵で ssh の標準入力へ流す。サーバー側では `bin/deploy.sh` が動く。ワークフローが送る文字列は `authorized_keys` の `command=` により無視される。
- 緊急：`bin/deploy.sh` は CI から tar を受け取る前提なので、人が単体で実行することはできない。GitHub に到達できないときや、直前の状態へ戻したいときは、この節の「失敗したときの巻き戻し」による。

**起動する人が確認すること**

`deploy.yml` は `workflow_dispatch`（手動起動）のみで、CI が緑かどうかを機械的には見ない。`ci.yml` に `deploy` ジョブを足して `needs:` で結ぶこともできるが、それは「毎回走る CI」と「押したときだけ走るデプロイ」を1つのファイルに同居させ、条件分岐を持ち込むことになる。分岐を避けるためにファイルを分けた。かわりに、**起動する人が CI の緑を確認してから押す**。これを手順とする。

**多重起動を防いでいる**

`deploy.yml` は同時に2つ走らないようにしてある（`concurrency`）。2つのデプロイが重なると、`git merge` と `composer install` と symlink の張り替えが交錯し、どの世代がどのコードに対応しているか分からなくなる。走っている途中のデプロイは止めず、後から来たものを待たせる設定にしている（途中で止めるほうが危険なため）。

【7-4e】この段落はかつて `npm ci` が `node_modules` を削除してから入れ直すことを重なりの理由として挙げていた。7-4e でビルドが CI へ移り、サーバーでは `npm` を実行しなくなったため、理由を上のとおり書き換えた。

**判定できる範囲**

`deploy.yml` が確認するのは `bin/deploy.sh` の終了コードまでで、サイトが実際に表示できることは含まない。理由はこの節の「スクリプトの設計判断」にある応答確認の扱いと同じで、Basic 認証の資格情報を CI に持たせないためである。7-9 で認証を撤去したあとに応答確認を `deploy.yml` へ足す（41-5）。

**鍵ごとに操作を絞る**

- CI 専用の鍵を1本追加し、サーバーの `~/.ssh/authorized_keys` でその行の先頭に `restrict,command="/var/www/portfolio/bin/deploy.sh"` を置いた。作業用の鍵はそのまま残っている。
- `command=` は、その鍵が認証に使われるたびに指定コマンドを実行し、**利用者が与えたコマンドは無視する**（`sshd(8)`）。無視されたコマンドは `SSH_ORIGINAL_COMMAND` から読めるだけで、`bin/deploy.sh` はそれを参照しない。
- `restrict` は、ポート転送・エージェント転送・X11 転送を無効にし、擬似端末の割り当てと `~/.ssh/rc` の実行も無効にする（`sshd(8)`）。
- 専用ユーザーを作らなかった。`sshd_config` に `AllowUsers` があり（7節）、ユーザーを増やすとそこに手が入る。さらに 22節のグループと setgid の設計にも影響し、7-4b で3本消した維持依存（40節）を増やす方向になる。`command=` は鍵ごとに効くので、ユーザーを分けなくても操作を限定できる。
- 絞る軸が違う。専用ユーザーは「触れるファイルの範囲」を絞り、`command=` は「実行できる操作」を絞る。この用途では後者のほうが絞りが強い。

**この方式が守らないもの**

`command=` が固定するのは**実行するファイルのパス**であって、そのファイルの中身ではない。`bin/deploy.sh` はリポジトリ内のファイルで、`git fetch` と `git merge --ff-only` でリポジトリの新しいコミットを取り込む。

- `main` に `bin/deploy.sh` を書き換えたコミットが入れば、次のデプロイでそれが本番で走る。
- `bin/deploy.sh` を書き換えなくても、`composer.json` の `scripts` はサーバー上の `composer install` で走り、PHP のコードそのものは php-fpm が実行する。いずれもリポジトリの中身で決まる。
- したがって、**`main` に書き込める者は、実質的に本番サーバーで任意のコードを実行できる**。これは設定ミスではなく、リポジトリの内容をサーバーで実行するという自動デプロイの構造そのものから来る。防御線はサーバーの入口ではなく、`main` に何が入るかに移る。
- `command=` が実際に守るのは、**鍵が漏れたときに、リポジトリを経由せず直接サーバーを操作されること**。
...
- 後ろ3つは秘密ではない。secrets に入れたのは、この文書が接続先とユーザー名を伏せている方針とワークフローの記述を揃えるためで、**秘匿が目的ではない**（公開しているドメインを DNS で引けば接続先は分かる）。
- 秘密鍵は**開発機で生成**し、サーバーには公開鍵だけを置いた。**サーバー上に秘密鍵は存在しない**。サーバーで生成すると、GitHub へ運ぶために秘密鍵を画面へ出力する操作が避けられなくなる。
- ワークフローでは `${{ secrets.X }}` を `run:` に直接展開せず `env:` を経由する。`${{ }}` はシェルの実行前にスクリプト本文へ文字列展開されるため、複数行の秘密鍵を直接置くとスクリプトの構造が壊れる。
...
| 確認 | 結果 |
| --- | --- |
| 送ったコマンドが無視されるか | 送った文字列は実行されず、`bin/deploy.sh` が完走、終了コード 0 |
| 擬似端末なしで `bin/deploy.sh` が通るか | 通った（`restrict` により PTY なし。それまでは対話シェルから実行していた） |
| ポート転送が拒否されるか | リモート転送の要求が拒否され、終了コード 255 |
| エージェント転送・X11・`~/.ssh/rc` | 個別には未実測（`sshd(8)` の記述による） |

### ビルドを CI へ移した（7-4e）

**なぜ移したか。** それまで `bin/deploy.sh` はサーバー上で `npm ci` と `npm run build` を実行していた。これは**他人が書いたコード（npm 依存とビルドツール）を本番サーバーで実行している**ということだった。7-4e で、ビルドを使い捨ての CI マシンへ移し、サーバーへは成果物だけを送る。あわせて `engines` の宣言（`^24`）と本番の Node（apt の 22）の食い違いも、Node がサーバーから無くなることで解消する（21節）。Node.js の実際の削除は、この方式が本番で成功したことを確認してから別手順で行う。

**ワークフローを2つのジョブに分けた。** GitHub ホストのランナーは、ジョブごとに新しい仮想マシンを立て、ジョブが終わると壊す（GitHub Docs。ジョブは互いにネットワーク的にも隔離される）。1つのジョブのステップはすべて同じマシンで動く。`deploy.yml` にビルドをそのまま足すと、`npm ci` が実行する他人のコードと秘密鍵が同じマシンに同居し、他人のコードが後続ステップの入力を差し替えたり、鍵が置かれるのを待ったりできる。ジョブを分ければ、他人のコードが動くマシンと、鍵があるマシンが別になる。

- `build` ジョブ：secrets を使わない。`npm ci` / `npm run build` を実行し、`public/build/` の中身と、ビルドしたコミットのハッシュ（`COMMIT`）を artifact にする。
- `deploy` ジョブ：secrets を使う。artifact を取得し、`npm` は一切動かさず、tar にして ssh の標準入力へ流すだけ。**成果物を実行しない**ので、汚染された成果物が鍵を盗む経路にはならない。

**secrets を持つジョブに、初めて他人のコードが入る。** ジョブ間でファイルを渡す手段は artifact しかなく、`deploy` ジョブは `actions/download-artifact`（サードパーティの action）を使う。7-4d までの「secrets を持つジョブには自分たちが書いたシェル以外を置かない」という原則を、ここで一段緩めた。緩めた範囲は「同一ワークフロー内での artifact の受け取り」に限る。

**成果物とコードの照合。** `build` ジョブは成果物に、ビルドしたコミットのハッシュを `COMMIT` として同梱する。サーバーは `git fetch` のあと、取り込もうとしている `origin/main` と `COMMIT` を突き合わせ、一致しなければ**何も変更せずに中止する**。`git pull` を `git fetch` と `git merge --ff-only` に分けたのはこのため。ビルドの後に `main` へ別のコミットが入っていた場合、古い成果物で新しいコードを動かしてしまうのを防ぐ。

**成果物の配置。** `public/build` は symlink で、`releases/<タイムスタンプ>/build` を指す。`ln -sfn` で新しい symlink を `public/build.new` として作り、`mv -T` で `public/build` へ被せる（`rename(2)` は不可分なので、切り替えの瞬間に中途半端な状態が見えない）。`releases/` は5世代を保持し、古いものから消す。`releases/` はリポジトリの中に置き、`.gitignore` に入れてある。

**成果物の信頼。** 成果物は CI が作る。CI のワークフロー定義（`build` ジョブの中身）と、そこで動く action・npm 依存を信頼している、ということをここに記録しておく。`COMMIT` の照合が保証するのは「成果物と、サーバーが取り込むコードが同じコミット由来である」ことだけで、成果物の中身が正しいことは保証しない。

### 配置方式について

リリースディレクトリ方式（PHP のコードごと世代で切り替える）を採らず、単一のディレクトリをその場で更新する。理由は、opcache の扱い・nginx の `root`・setgid の権限モデルに広く手を入れることになり、規模に見合わないため。7-4e でビルド成果物だけは世代で持つが、これは `public/build` に限った例外である。PHP のコードは `public/build` に含まれないため、opcache・`root`・権限モデルのいずれにも影響しない。

（この判断について、当初検討した経緯の記録は残っていない。上はいま改めて成り立つ理由を書いたものである。）

### 既存サーバーの移行（`public/build` を symlink にする）

サーバーを新規に構築する場合は不要。7-4e より前から動いているサーバーでは、`public/build` が実体のディレクトリなので、`bin/deploy.sh` は「配置の確認」で止まる。一度だけ次を行う。
...
PHP-FPM の各ワーカーは、解決したパスを `realpath_cache_ttl`（既定120秒）の間キャッシュする。symlink を張り替えた直後、キャッシュが切れるまでの間は、ワーカーによって古い世代の `public/build/` を読むことがある。古い世代のファイルは消さずに残してあるので**ファイルは存在し**、キャッシュが切れれば新しい方に切り替わる。**壊れるのではなく、切り替わりが遅れる。** nginx が `public/build/` 配下の静的ファイルを直接返す経路は PHP を通らないため、この遅れの影響を受けない。
...
| 全体を `{ }` で囲む | `git merge` がこのファイル自身を書き換えても、bash が読み進める途中で内容が変わらないようにするため。bash は `{ }` の中を先に読み切ってから実行する。囲む代償はゼロで、囲まない場合の代償はデプロイが壊れることなので、真偽を検証せず広いほうへ倒した（16節・39節と同じ非対称の判断） |
| `git fetch` と `git merge --ff-only` に分ける（`git pull` を使わない） | (1) `fetch` して `origin/main` を成果物のコミットと照合してから `merge` する。取り込む前に「成果物と同じコミットか」を確かめられる。(2) 素の `git pull` は、サーバー側とリモートが分岐しているとマージを作る。サーバーではコミットしない約束だが、その約束が破れたときに黙ってマージするのではなく止める。前提を仮定ではなく強制にする |
| 標準入力を最初に読み切る | 送り手（CI 側の `tar czf - \| ssh`）は、こちらが読み終えるまでブロックする。他の工程より先に `tar xzf -` で受け切る |
| 作業ツリーのクリーン確認と `main` ブランチの確認 | サーバー上で手作業の編集や診断用ファイルの置き忘れ、あるいは detached HEAD の状態があると、`git merge` が失敗するか、意図しない状態でデプロイする。実際に一時ファイルを置く場面が繰り返し発生している |
| 成果物のコミットと `origin/main` の照合 | 一致しなければ**何も変更せずに中止**する。ビルドの後に `main` へ別のコミットが入った場合に、古い成果物で新しいコードを動かすのを防ぐ |
...
【7-4d】上の「サイトの応答確認を含めない」は、CI から起動する `deploy.yml` にもそのまま適用した。`deploy.yml` が確認するのは `bin/deploy.sh` の終了コードまでである。詳細はこの節の「GitHub Actions からの起動（7-4d）」。

### 失敗したときの巻き戻し

デプロイが途中で止まっても、自動では巻き戻らない（理由は上の「スクリプトの設計判断」を参照）。`bin/deploy.sh` は symlink を張り替える前に止まれば `public/build` は前の世代のままで、サイトの見た目には影響しない。symlink を張り替えた後・キャッシュ生成の途中で止まった場合は、新しいコードに古い（または一部だけ新しい）キャッシュが載る状態になりうる。手で戻す経路は次のとおり。

**主経路：revert して、通常どおりデプロイする**

GitHub 側で問題のコミットを revert して push し、CI が緑になったら Actions から `Deploy` を起動する。サーバーが常に `origin/main` を追うという性質が保たれるため、通常のデプロイがそのまま巻き戻しになる。**原則としてこちらを使う。**

**ビルド成果物だけを前の世代へ戻す：`bin/rollback.sh`**

直前のデプロイでフロントの見た目が壊れた（PHP のコードは変えていない）、あるいは主経路を待てないときに使う。人がサーバーに入って実行する。

- `./bin/rollback.sh` … 保持している世代を、それぞれどのコミットのものかとあわせて一覧表示する
- `./bin/rollback.sh <世代>` … `public/build` をその世代へ張り替え、キャッシュを作り直す

`bin/rollback.sh` が戻すのは `public/build`（ビルド成果物）だけである。**PHP のコードは戻らない。**

**PHP のコードごと戻す**

コードに問題があり、主経路も待てないとき。Node は不要。
...
./bin/rollback.sh <その頃の世代>   # 資産もその頃のものに合わせる場合
```

`git checkout` で detached HEAD になる。次の通常デプロイは `bin/deploy.sh` が `main` にいることを確認するため、この状態では動かない。復帰するときは `git checkout main` してから、Actions で `Deploy` を起動する。`git reset --hard` は使わない（次の `git merge --ff-only` が最新まで早送りして巻き戻しを取り消す）。

**この節に書かれている主経路（revertしてデプロイし直す）は、一度も実行していない**（41-7）。**手順が書かれていることは、使えることの証明ではない。** 3節の方針に照らすと、この備えは「確認済み」の水準に達していない。確認するには壊れたデプロイを1回行う必要があるため、現時点では行わないと決めた。

**1回のデプロイで取り込まれるコミットは、1件とは限らない。** デプロイは`git merge --ff-only`で取り込むため、**前回のデプロイ以降に積んだコミットがすべて入る。** 実測（2026-09-26）：本番のHEADが数枚前のカードのコミットだったため、その後に積んだ文書の変更と、当日の修正がまとめて入った。

**したがって、デプロイ後に問題が出たとき、原因が直前のコミットだとは限らない。** デプロイの前に、**本番の現在のコミットを記録しておくこと**（`git log --oneline -3`）。

**あわせて、`git status --short`が空であることも確認すること。** 追跡しているファイルに変更があると、`git merge --ff-only`がその段階で失敗する。実測（2026-09-26）：空であることを確認してからデプロイし、成功した。
...
【7-4e】`npm ci` / `npm run build` は 7-4e で CI へ移した。サーバーの `bin/deploy.sh` はビルドをせず、CI が作った成果物を受け取るだけになった。詳細はこの節の「ビルドを CI へ移した」。
...
**【後日の訂正（7-5b）】上の確認のうち、問い合わせフォームが正常に動作したという部分は、成立していなかった。**

実測（2026-09-23）：本番の`laravel.log`に残っている問い合わせフォームの記録は4件で、**4件すべてがスパム判定で送信されずに終わっていた。そのうち2件は2026-08-23、すなわち上の確認を行った当日のものである。** 送信元のIPアドレスとブラウザはいずれも同一で、作業者自身の送信である。

**当時の確認は、画面に成功メッセージが出たことを、フォームが正常に動作した証拠として使っていた。** スパム判定は送信せずに成功を返す設計であるため、**画面の表示からは区別できない。** 当時は`MAIL_MAILER=log`であり、届かないのが当然の状態だったため、届かないことからも気づけなかった。**そして判定で止まった場合、問い合わせの内容はログにも残らない。**

**動作確認が教えてくれるのは、今の組み合わせで動くかだけである。画面に成功メッセージが出たことは、送信が行われた証拠にはならない。** この一件は、その実例が2ヶ月にわたって記録に残っていたものである。

**原因は未確定である。推測：判定が値の有無ではなくキーの有無を見ているため、空の隠しフィールドが常に判定に掛かっている。** 4件中4件という結果とは整合するが、**実装を読んで確認していない。7-5cで扱う。**

**【後日の解消（7-5c-1）】** 実測（2026-09-26）：原因を特定して修正し、デプロイしたうえで、**本番のフォームから送信してメールが届くことを確認した。** 同時に、本文にURLを3つ入れて送ると利用者にエラーが表示され、メールが送られないことも確認した。

**原因は、判定が「入力が空のときは空文字列で届く」ことを前提にしていたのに対し、Laravelのグローバルミドルウェアが空文字列を`null`に変換していたことである。** 判定の式は単体では正しく、ミドルウェアも既定の動作であり、**どちらも単体では誤っていなかった。誤っていたのは、入力の形についての前提である。**

**当時の確認の形も記録する。** 入力は2通り（囮のフィールドを埋める／埋めない）用意していた。**しかし結果を「エラーが出ていないこと」で読んだ。エラーの有無は2通りのどちらでも同じであり、2通りを区別できない場所だった。** 区別できる情報は同じログの中にあった——スパム判定の記録が2件書かれていた。

**対照実験を組んでも、結果を読む場所が両者を区別しないなら、実験は何も教えない。**
...
`ci.yml` に、`ubuntu-latest` ランナーへ最初から入っている shellcheck を使うステップを足した。インストールのステップは無い。閾値は `--severity=warning` にしている（理由は下記）。`bin/*.sh` に `shellcheck disable` の抑制注釈は無い。理由は普通の日本語コメントとして残している。

- `bin/deploy.sh` の `cleanup()`：直接は呼ばれず `trap cleanup EXIT` 経由で呼ばれる、という趣旨のコメントがある（SC2329 が指摘する内容についての誤検出の記録）
- `bin/deploy.sh` と `bin/rollback.sh` の `ls -1d`：世代名は `date` が作る数字とハイフンのみで、SC2012 が警戒する空白・改行を含む名前にはならない、という趣旨のコメントがある。形式はこの節の「スクリプトの設計判断」に定めている

**開発機で走らせる shellcheck と、CI（`ubuntu-latest`）内蔵の shellcheck は、版が違いうる。** 実測（2026-09-15）：開発機（Docker、`koalaman/shellcheck:stable`）は 0.11.0、CI のランナーが内蔵する版は `actions/runner-images` の公開資料上 0.9.0-1（CI が実際に使った版そのものをログから読んだわけではない）。

この違いが実際に問題を起こした。版が違うと、同じ問題に対して異なる検査 ID・異なる行が報告されることがある。実測（2026-09-15）：開発機の 0.11.0 は `cleanup()` の関数定義行に SC2329（「この関数は呼ばれていない」）を報告したが、CI 側は SC2329 を持たず、代わりに `cleanup()` 内の `rm -rf "$incoming"` の行に SC2317（「到達不能に見えるコマンド」）を報告した。**SC2329 の抑制注釈は SC2317 を抑えない。** SC2317 用の注釈を足す案は、以後も両方の ID を追いかけ続けることになるため採らなかった。

（当初「新しい版で SC2317 が分割され、関数には SC2329 が割り当てられた」という理解で書いていたが、これは確認していない推測だった。shellcheck の CHANGELOG を確認したところ、SC2329 は v0.11.0（2025-08-03）で新設された別の検査であり、SC2317 は同じ v0.11.0 で「入れ子のコマンドに対して過剰に報告しないよう調整された」——CHANGELOG 原文は "SC2317 about unreachable commands is now less spammy for nested ones"——という、分割ではなく別々の変更だった。出典：[shellcheck CHANGELOG.md](https://github.com/koalaman/shellcheck/blob/master/CHANGELOG.md)。）

**したがって、開発機で0件になっても CI で0件になるとは限らない。** info レベルの検査は、版によって出たり出なかったりする。ゲートの基準が版で揺れるなら、ゲートとして成立していない。7-4c の「ゲートの基準は届く先で決まる」に照らすと、error / warning が届く先は「デプロイが壊れる」、info が届く先は「読みやすさ」「将来の耐性」である。実際に出た3件（SC2329・SC2012 × 2）は、いずれもこのリポジトリには当たらないものだった。**したがって閾値を `warning` にし、info は見ない。**

info を見ないことの代償は、将来の info が検出されなくなることである。実際に出た3件がいずれも当たらなかったという経験にもとづく判断であり、当たる info が出ないことを保証するものではない。

**ランナーイメージの更新で shellcheck の版が変わりうる、という性質もそれ自体は正しい。** ただし今回起きたのはそれとは別の原因（開発機と CI が最初から異なる版を使っていたこと）である。両方とも「版によって info の内容が揺れる」という同じ結論を支える、別々の事実として残す。

**shellcheck が捕まえるのは構文で、論理の誤りは捕まえない。** 7-4e で見つかった欠陥（古い世代の掃除の失敗を終了コードへ畳んでいた）は、構文としては正しく、shellcheck では検出されない。
...
- **apache2-utils（htpasswdコマンド）は導入しない。** 既に入っているopensslで生成したSHA-512 crypt形式をNginxが検証できることを実測で確認した。導入するパッケージを増やさない方針（13節：Apache関連を避けた判断、およびインストールするソフトウェアが減れば攻撃面と更新対象が減るという理由）と整合する
...
共存することは実測で確認した。apexとwwwのAレコードは設定2に存在し、apexのMXとSPFのTXT、および`_dmarc`のTXTは設定2に存在しないが、いずれも実際に引ける。後者3本は設定1から生成されている。

実測（2026-09-21）：設定2の画面に並ぶ行は6本（apexのA、wwwのA、newのA、`resend._domainkey`のTXT、`send`のMX、`send`のTXT）。一方、権威サーバーに引けるレコードは、この6本に加えてapexのMX、apexのTXT（SPF）、`_dmarc`のTXTがある。差の3本はいずれもロリポップのメールサービスに関わるレコードであり、設定1が1つの束として生成していると考えられる（推測。確認していない）。

**管理画面と権威サーバーは、別の問いに答える。** 設定2の画面が見せるのは「手で書いたレコード」であり、設定1が生成したレコードは現れない。したがって、レコードが存在するかどうかの判定には、常に権威サーバーへの問い合わせを使う。管理画面は「何を書いたか」を、権威サーバーは「何が効いているか」を答える。
...
### Resendのレコードがapexと`send.`に分かれている理由（7-5a）

設定2の3本は、apexと`send.`サブドメインの2箇所に分かれている。役割が違うためである。

| レコード | 置き場所 | 担っているもの |
|---|---|---|
| `resend._domainkey` TXT | apex | DKIMの公開鍵。署名するドメインがapexになるため、Fromヘッダをapexにしてもアラインメントが取れる |
| `send` TXT（SPF） | `send.`サブドメイン | 封筒の差出人（envelope sender / Return-Path）のドメインに対するSPF |
| `send` MX | `send.`サブドメイン | 配信不能通知（バウンス）の受け取り |

出典：Resendは既定でドメインの`send`サブドメインをReturn-Pathに使う。"By default, Resend uses the `send` subdomain for the Return-Path address."（Resend公式ドキュメント Configure a custom Return Path。2026-09-21取得）

**この分離により、apexのTXT（SPF）に手を入れずに済む。** SPFの照合対象は封筒の差出人のドメインであり、封筒の差出人が`send.ikshowcase.site`である限り、apexのTXTは引かれない。apexのTXTはロリポップ向けのSPFを宣言しているため、書き換えるとロリポップ側から送られたメールの扱いが変わる。

**Fromヘッダのドメインはapexにする。** DKIMの公開鍵がapex直下にあり、署名ドメインがapexであれば、DMARCのアラインメントがDKIM側で成立する。**ただし署名ドメイン（`d=`の値）は、確認した公式文書の範囲では明示的な記載が見つからなかった。DNSの鍵の置き場所からの推測である。届いたメールのヘッダで実測すること**（7-5bで行う）。

実測（2026-09-21）：`_dmarc`のTXTは`v=DMARC1; p=none;`である。`p=none`は、認証に失敗しても特別な扱いを求めないという指定である。**`rua=`（レポートの送り先）が含まれていないため、DMARCの判定結果はどこにも報告されない。設定は存在するが、観測には使えない。**

実測（2026-09-21）：`send`のMXは`10 feedback-smtp.ap-northeast-1.amazonses.com.`、`send`のTXTは`v=spf1 include:amazonses.com ~all`である。バウンスの受け取りとSPFの許可が、Resendのドメインではなく`amazonses.com`を指している。**推測：ResendはAmazon SESを送信の基盤として使っていると考えられる。Resendの公式文書では確認できていない。** この推測が正しい場合、依存の連鎖は「アプリ → Resend → Amazon SES → 受信側」となり、Resend自身の障害だけでなくAmazon SES側の障害も経路に入る。

**【追記（7-5b）】上の推測に、実測の裏づけが3件加わった。** 実測（2026-09-23）：本番から1通送信し、届いたメールのヘッダを読んだところ、(1)`X-SES-Outgoing`のヘッダが存在した、(2)`Received:`の経路に`smtp-out.ap-northeast-1.amazonses.com`が現れた、(3)**DKIM署名が2つ付いており、片方の`d=`が`ikshowcase.site`、もう片方が`amazonses.com`だった。両方ともpassしている。** **ただし、Resendの公式文書で確認できていないことは変わらない。** 実測が示しているのは、この1通がAmazon SESの設備を通ったことであり、Resendの構成そのものではない。
...
- **この検証の要点は、足した行が見えることではなく、足していない行が変わっていないことである。** 変更前の状態を先に記録し、後で照合する形にしたのは、apexのMXとTXT（SPF）で壊れたときの現れ方が違い、一方が静かに壊れるためである。
  - **MXが消えると、apex宛のメールの受け取り先が分からなくなり、受信が止まる。**
  - **SPFは消えても送信は止まらない。** 照合結果が`None`になるだけである（出典：RFC 7208 §2.6.1・§8.1。2026-09-21確認）。
  - **消えるよりも、誤った内容に書き換わる方が危険である。** 結果は`Fail`（§8.4）または`Softfail`（§8.5）になり、受信側は拒否も迷惑メール扱いもできる。
  - **いずれの場合も、送った側にエラーは返らない。** ロリポップのサーバーは送出に成功し、受信側で静かに扱いが変わる。**この静かさが、変更前の記録と照合を必要にしている。**

【訂正（7-5a）】当初この理由を「MXとSPFが消えるとメールが停止するため」と記載していたが、SPFについては誤りである。SPFは送る側の許可証ではなく、受け取る側への申告であり、消えても送信は止まらない。**判断（変更前の状態を記録して照合する）は維持する。訂正が必要なのは理由の範囲のみである。**
...
代償：snapは自動更新されるため、予期しないタイミングで版が上がる。1節でスタートアップスクリプトを不使用としたのと同じ方針（入るものを自分で把握できる状態に保つ）と正面からぶつかる。それでもsnapを選んだのは、止まった瞬間にサイトが開けなくなる種類の道具では、「古いまま止まる」ほうが「勝手に新しくなる」より危険だと判断したため。
...
更新のたびに実体が`2.pem` `3.pem`と増え、リンクだけが最新を指し続ける。37節で本番更新を実際に行い、確認済みである。Nginxにはリンクの側を書く。
...
33節に記録した、HTTPS化前は資格情報が経路上を平文で流れるという問題は、この移動によって解消する。
...
8節に、ポートを開けるときはConoHaのセキュリティグループとufwの両方を確認する必要があると書いた。今回はConoHa側が既に開いており、確認したうえでufw側だけ触ったという記録である。
...
- 3節と同じ判断である。備えは、存在することではなく、動くことを確認済みであることを指す
...
3節で、備えは原因ではなく結果（入れない状態）に対して設計するとしたのと同じ考え方である。
...
- 9節で、気づいたら手動で再起動する運用は続かないと判断したのと同じ思想である。新しい習慣を作らず、既にある動作に乗せる
...
3回目は、36節の443番の受け皿（`ssl_reject_handshake`）を利用している。存在しない名前で接続すると暗号化の約束が断られ、証明書が取れない。サービスを止めずに異常系を試せる。
...
bootstrap/cache/config.php
   ...
   └─ Laravel既定のメールレイアウト（メール本文のリンク）（推測）
```

**実測（2026-09-23）：本番から送ったメールの本文（テキスト版とHTML版の両方）に、`APP_URL`と同じURLがリンクとして現れた。推測：Laravelの既定のメールレイアウトが、この値をリンク先に使っている。コードを読んで確認していない。**
...
明示する理由は、19節で、本番に必要な値は .env.example の既定値に依存せずすべて明示的に設定するとした方針に従うためである。

この変更の効果は、現在の構成では観測できない。80番が転送専用であり、Laravelが暗号化されていない経路で応答することがないためである。目的は観測できる変化ではなく、依存を1つ減らすことである。

**【追記（7-5b）】`SESSION_SECURE_COOKIE`を`.env.example`に足さないと判断した。** 理由：`.env.example`は開発環境のテンプレートであり、**本番固有の設定の台帳は19節である。両方に置くと、値が変わったときに2箇所を直すことになり、片方が古くなる。** 実測（2026-09-23）：本番の`.env`と`.env.example`のキーの差は、この1件だけである。
...
13節で推測による先回りを避けたことには当たらない。13節は、必要になるかどうか分からないものを入れなかった判断であり、今回は必要になることが確定している。
...
**【後日の訂正（7-5b）】上の、30節の権限の再適用が必要であるという記述は、現在は成立していない。** 7-4bで権限の設計を変えた際に、**30節の権限の再適用は廃止された。7-4bの変更が、この節に反映されていなかった。** そのため、この節を読んで`config:cache`が権限を緩めることを知った人が、**存在しない手順を探すことになる。**
...
- 実測：`npm run build`の成果物のファイル名は変わらなかった（`app-CYEMFStJ.js` / `app-BqLPbuc1.css`）。フロント側を変更していないため。21節で実測した、ビルド成果物が環境をまたいでも一致するという結果の、時間をまたいだ版にあたる
...
- 未確認：再利用が`ssl_session_cache`（サーバー側のキャッシュ）で成立しているのか、セッションチケットで成立しているのかは切り分けていない。どちらであっても本節の設定の判断は変わらないため、検証していない
...
【訂正（7-4a-1）】上の「正しい理由：Permissions-Policyが効く場面は、CSPが効く場面と同じ（スクリプトの注入）である」も誤りである。

出典：MDNはPermissions-Policyを "provides a mechanism to allow and deny the use of browser features in a document or within any `<iframe>` elements in the document" と定義する（https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Permissions-Policy ）。CSPについては "The primary use case for CSP is to control which resources, in particular JavaScript resources, a document is allowed to load."、"This is mainly used as a defense against cross-site scripting (XSS) attacks, in which an attacker is able to inject malicious code into the victim's site." とする（https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/CSP ）。

この2つの定義から言えること：CSPはリソースの読み込みとスクリプトの実行を制限し、Permissions-Policyは機能APIへのアクセス可否を制御する。両者が関わるのは同じ脅威（注入されたスクリプト）だが、効く場面（制御ポイント）は別である。7-3d-4の訂正は前段（守る相手は注入されたスクリプトである）だけを直しており、後段（効く場面が同じ）は不正確なまま残っていた。

判断を「7-9でCSPと一緒に判断する」から「入れない」に変更する。入れない理由と、理由が二度誤った経緯は 41-7 に記載する。判断（入れない）自体は初回から変わっていない。
...
実測：同じ位置に置いた`auth_basic`は継承される。`location ^~ /build/`に`auth_basic off`を書いていないため、serverブロックの`auth_basic`がそのまま効く（`/build/`配下を`-u '<USER>'`付きで検証しているのはこのため）。`add_header`だけがlocation側の記述で置き換わり、`auth_basic`は残る。`nginx -t`はどちらの違いも検出しない。

（このときの確認コマンドは記録していない。再確認する場合は、`/build/` 配下と `/` のレスポンスヘッダを比較する）
...
上の「定期的に発生するもの」と「維持依存の索引」は、いつ何をするか（行為）を扱っている。ここでは、サーバーの上で動き続けているもの、または外部のサービスと結びついているもの（存在）を一覧にする。何があるかを忘れないための一覧であり、対処や判断はここには書かない。

| 何が | 節 | 止まったとき／失われたときに何が起きるか |
| --- | --- | --- |
| Nginx | 12・25・33・36 | サイトに到達できなくなる（推測、未確認） |
| PHP-FPM | 13・14・16・26〜30 | Nginxは応答するが、PHPが実行できずエラーが返る（推測、未確認） |
| sshd | 7 | SSHで入れなくなる（推測、未確認）。代替経路（コントロールパネルのコンソール機能）は3節に記載 |
| ufw | 8・12・36 | ポートの制限が外れる（推測、未確認）。ConoHaのセキュリティグループが別の層として残ることは8節に記載 |
| snapd（certbotの実行基盤） | 35 | certbotが動かず、証明書が自動更新されなくなる（推測、未確認） |
| ConoHa VPSの契約とセキュリティグループ | 1・8・12・36・40 | この節の「定期的に発生するもの」に記載 |
| ムームードメイン／ムームーDNS | 34 | 名前が引けなくなり、ドメイン名でサイトに到達できなくなる（推測、未確認）。証明書の更新（35節のHTTP-01チャレンジ）も名前解決に依存するため、影響を受けうる（推測、未確認） |
| Let's Encrypt（証明書の発行元） | 35・37 | 更新が続けて失敗すると、無人のまま証明書が失効し、ブラウザが警告を出す（37節）。再発行を繰り返すとレート制限に達しうることは35節に記載 |
| GitHub（リポジトリ・Actions・Rulesets・secrets） | 30 | mainへの書き込みが本番の実行権限に直結すること、GitHubに到達できない場合の代替経路は30節に記載 |
| サードパーティのGitHub Actions | 21・30 | CIとデプロイが動かなくなる（推測、未確認） |
| Resend（メール送信） | 19・34・41-3 | メールが送れなくなる。問い合わせの通知が届かない |
| kdump-tools | 41-7 | 実測（2026-09-23）：`active`で常駐している。**この一覧に漏れていた。** 推測：26節が記録しているメモリの実数と、契約している容量との差は、これによる予約である。**確認していない**（41-7） |

**Resendについての補足（7-5b）**：2026-09-23から、本番のメール送信に実際に使用している。接続はSMTPで、`smtp.resend.com`のポート465である（19節）。**APIキーが`.env`と`bootstrap/cache/config.php`に入っており、実測（2026-09-23）：Resendのキーの作成画面に有効期限の項目が無い。したがってキーは、こちらが差し替えるまで有効であり続ける。** 送信の経路にAmazon SESが入っている（34節）。**アカウントには複数のキーが存在し、全権限のキーが1本でもある限り、ドメインを限定したキーによる緩和は効かない**（15節、7-10）。
...
- ライブラリ（libcurlなど）が更新されると、needrestartが影響を受けるサービスの再起動を扱う。**扱い方は3通りあり、どれになるかは出力を見ないと分からない。**
  - **自動で再起動する。** php8.5-fpmやnginxがこれにあたる。サービスの起動時刻が変わっていたら、この可能性を疑う
  - **再起動を先送りする。** 実測（2026-09-21）：`apt upgrade`の出力に`Service restarts being deferred:`の行が現れ、dbusの再起動が先送りされた。**このとき`/var/run/reboot-required`は立たない**（同日、該当なしを実測）。needrestartによる自動再起動も、9節が定める自動再起動も発動しないため、**次に何らかの理由でサーバーを再起動するまで、先送りされたサービスは古い状態で動き続ける。気づく経路は無い。** 実測（2026-09-23）：同じ行が再び現れ、このときは3件（dbus・networkd-dispatcher・unattended-upgrades）が先送りされた。**同日の再起動で3件とも解消した**
  - **カーネルについては、自動では扱わない。** 実測（2026-09-23）：カーネルが更新された回では、needrestartは再起動を検討するよう診断を出すだけで、何も再起動しなかった。**ただしこのときは`/var/run/reboot-required`が立つ**ため、9節が定める自動再起動の対象になる。先送りの場合と異なり、**気づく経路がある**

  推測：先送りされる理由は出力に現れない。dbusのように多くのサービスが依存する土台では、自動再起動の影響が大きいためだと考えられる。確認していない。

  **したがって、`apt upgrade`の出力は最後まで読むこと。**`Service restarts being deferred:`の行が現れた場合、先送りされたサービスを41-7に記録し、次に再起動する機会を意識すること。**この手順は2026-09-21と2026-09-23の2回、実際に機能した。**
...
ある節が達成した状態が、別の節の手順を実行し続けなければ保たれない関係がある。実行が止まらないため、失敗しても気づきにくい。以下はその一覧である。**復旧する手順はこの表に書く。** 索引を見るのは「失われた」と気づいたときであり、そのとき手順が手元にある方が使えるためである。判断の理由（なぜその形を選んだか）は元の節に置く。
...
`21節 → （回復手順なし）`（Node と npm の版が上がると、ビルド成果物が環境をまたいで一致するという実測が成り立たなくなる）の1本は、7-4e で解消した。ビルドを行う場所が CI の1か所になったため、環境をまたいで一致するという実測の要件そのものが無くなった（自動化ではなく消滅）。かわりに 7-4e で `ci.yml` と `deploy.yml` の build を揃える依存が1本増えた（上の表）。
...
**並べ方の方針**：発生順ではなく**回収先ごと**に並べる。「いつ判明したか」より「いつ扱うか」で引くほうが、カードの開始時に読みやすいため。

カードごとの発見ログは統一ログに残す。**この節は台帳を1つに保つ。** 同じ項目を複数の見出しに載せない。

---

### 41-1. 7-4a（手順書の監査）で回収した

4項目すべてを 7-4a-1 と 7-4a-2 で解決した。詳細は 41-9。

---

### 41-2. 7-4f で回収する

`opcache.validate_timestamps = 0` の再検討を 7-4f で解決した。詳細は 41-9。

---

### 41-3. 7-5（本番メール）で回収する

| 項目 | 補足 |
| --- | --- |
| Resendのレコードの構成（`send`と`resend._domainkey`）と、引き継ぎ資料の`mail.ikshowcase.site`という記載の不一致 | **7-5aで決着。** apexと`send.`サブドメインに役割を分ける構成がResendの標準であり、これで動く（34節）。「どちらに寄せるか」という選択は存在せず、引き継ぎ資料の記載が誤っていた。**41-3の残る3件が7-5dで片付いた時点で、4件まとめて41-9へ移す** |
| 証明書の更新失敗を知らせる通知の経路が無い | 現在の検知はログイン時の表示と月次の確認に依存しており、気づくまでに最大30日かかりうる。メールが使えるようになったら、失敗そのものを通知する形へ再設計する |
| snap による certbot の版の変化は検知していない | 版が変わって更新が壊れた場合、残り日数の検知が拾うが、気づくのは遅い。上と同じ再設計で解決する |
| 通知を certbot の終了コードに紐づけない | 実測：deploy hook が失敗しても certbot の終了コードは 0 になる。**サーバーが提示している証明書を見る形にすること**（37節と同じ考え方） |

---

### 41-4. 7-7（画像の準備・戦略）で回収する

| 項目 | 補足 |
| --- | --- |
| `/images/` にキャッシュの指示が無い | ファイル名にハッシュが無いため長期の期限は付けられないが、何も付けないままでよいかは判断していない（39節と対になる判断） |

---

### 41-5. 7-9（apex への本切り替えと本番最終確認）で回収する

**同じカードの中では、公開を先に、監視を後に行う。** 監視は公開後の状態を対象とするため。

#### 切り替えの作業

| 項目 | 補足 |
| --- | --- |
| Basic 認証の解除 | 現在は `new.ikshowcase.site` を Basic 認証の内側で公開している（33節・34節） |
| `deploy.yml` に応答確認ステップを足す | Basic 認証を撤去したら、デプロイ後に公開 URL を引いて表示できることを確認するステップを `deploy.yml` に追加する。引くパスは `ci.yml` の本番構成チェックと同じ6本を想定。30節の、応答確認を含めないという判断は、この時点で解除される（7-4d で追加） |
| Basic 認証のユーザー名が OS のユーザー名（SSH の `AllowUsers` 対象）と同一 | 401 はユーザー名を明かさず、存在するユーザー名と存在しないユーザー名で応答も一致する（根拠は表の下）。SSH はパスワード認証を無効化済みのため実害は小さいが、7-3a で「ありふれない名前を選ぶ」とした判断とは整合しない。**認証の撤去で解消する** |
| apex への切り替え | **ムームー DNS の設定2 の A の行の内容を書き換えるだけで済む。設定1 は触らない**（34節） |
| `new.ikshowcase.site` の廃止方法 | レコードを削除するか、apex への転送を残すかを決める |
| `APP_URL` を apex に変える | canonical・`og:url`・sitemap・robots がすべて追随する。**コードの変更は不要**（38節） |

**「401 はユーザー名を明かさず」の根拠**

出典：401 応答（チャレンジ）に含まれるのは認証スキームと realm（必須）、および任意の charset のみで、ユーザー名は含まれない（RFC 7617 §2。https://datatracker.ietf.org/doc/html/rfc7617 ）。

実測（2026-09-02）：存在するユーザー名と存在しないユーザー名で、401 の応答は完全に一致した（ステータス、`www-authenticate`、`content-length` = 172、本文）。

```bash
curl -si -u '<USER>:wrong-password' https://new.ikshowcase.site/
curl -si -u 'nosuchuser:wrong-password' https://new.ikshowcase.site/
```

未検証：応答時間による区別は確かめていない。サーバー上で 5 回ずつ測定したところ、中央値は存在する側 0.0176 秒、存在しない側 0.0147 秒で、差は約 2.9 ミリ秒。方向と大きさは SHA-512 crypt の計算時間として説明できるが（推測）、範囲が完全に重なっており（0.0157–0.0360 対 0.0144–0.0285）、TLS ハンドシェイクのばらつきと分離できていない。この結果は判断（7-9 の認証撤去で解消する）を変えないため、これ以上は測らない（6-4）。

#### 切り替え時に判断すること

| 項目 | 補足 |
| --- | --- |
| HSTS を入れるかどうか | `new.` では「得るものが小さい」として見送った |
| HSTS に `includeSubDomains` を付けるかどうか | 付けると、apex のサブドメイン（現在10本、すべてロリポップ側）にも同じポリシーが適用され、https でしか接続できなくなる（出典：RFC 6797 の `includeSubDomains` ディレクティブ（§6.1.2）。https://datatracker.ietf.org/doc/html/rfc6797 ）。ポリシーは受け取ったブラウザ側に `max-age` の期間保持されるため、**取り消すには `max-age=0` を配信し直す必要があり、その配信を受け取らないブラウザには届かない。** 10本の https 対応状況を確認したうえで判断すること。**【注記（7-5a）】「10本」が何を数えたものかは記録されていない。** 7-5aで、`send.ikshowcase.site`・`_dmarc.ikshowcase.site`・`resend._domainkey.ikshowcase.site`という名前が存在することを権威サーバーへの問い合わせで実測した（2026-09-21。34節）。**これらが10本に含まれているかは不明である。** 推測：10本はHTTPでアクセスできるサブドメインを数えたものであり、メールのための名前は数えていない。確認していない。**7-9で判断する前に、10本が何を数えたものかを確かめ直すこと。** |
| CSP の設定 | 許可すべき読み込み先は、実コンテンツと画像が入った後でないと確定しない（`Permissions-Policy` は 41-7 で「入れない」と確定した） |
| 監視の設定 | 40節を参照。監視が要るのは「落ちて困る状態」になったときであり、7-3d の時点では対象が存在しなかった |

> 証明書の自動更新が失敗した場合、それが表面化するのは証明書の有効期間が切れたときである（出典：有効期間は90日。37節に出典を記載）。加えて、失敗を知らせる通知は設定していない（実測：37節）。このため、**その検知手段だけは監視全般と切り離して先に用意した**（37節）。
...
| 項目 | 期限 |
| --- | --- |
| 証明書の残り日数の検知の閾値20日は、有効期間90日を前提にしている。**有効期間が64日になるより前に見直すこと**（出典：35節・37節に記載） | 2027-02-10 |

---

### 41-7. 回収先を持たない記録

**後続のカードで扱う予定は無いが、判断の前提として知っておくべき事実。**

| 項目 | 種別 |
| --- | --- |
| 古い証明書は `archive/` に残り、失効操作をしない限り notAfter まで有効なままである（出典：RFC 5280 §4.1.2.5）。更新は旧証明書の失効を伴わない（出典：Let's Encrypt の失効は鍵漏洩時などに ACME 経由で行う別操作。https://letsencrypt.org/docs/revoking/ ）。対応する秘密鍵のファイルも同じディレクトリに残る（秘密鍵自体に期限は無い） | 対処しないと決めたこと |
| `SESSION_SECURE_COOKIE` を明示したが、現在の構成では効果を観測できない。80番の構成を変えた場合に意味を持つ | 観測できない設定 |
| クライアント側の画面遷移では canonical と `og:url` が更新されない。推測：クローラは各 URL を個別に読み込み、クライアント側の遷移を経由しないため、検索エンジンには影響しないと考えている（確認していない）。**SSR を採用する判断をした場合に再検討する** | 条件付きの再検討 |
| `umask 027` のもとで composer が新しいファイルを作る場面が未検証（30節）。**次に依存が変わるデプロイで確認する。** **【注記（7-5b）】この検証の機会は、当面来ない見込みである。** 7-5bで送信方式をSMTPにしたため、**パッケージの追加が発生しなかった。次にcomposerで依存を増やすときまで、この項目は検証されないまま残る。** **【注記（7-5c-1）】実測（2026-09-26）：アプリのコードを変えたデプロイを行ったが、`vendor/composer/autoload_classmap.php`の更新日時は変わらなかった。この場面は発生しなかった。** 推測：依存の一覧が変わっていないため、Composerが内容の同じファイルを書き直さなかった。**確認していない。** したがってこの項目は、**依存が変わるデプロイのときにしか観測できない。** なお同じデプロイで`bootstrap/cache/config.php`は640・グループwww-dataで作り直されたため（実測 2026-09-26）、**デプロイ時のumask 027そのものは効いている。** | 条件付きの再検討 |
| 26節が記録しているメモリの実数と、契約している容量との差（87MB）が説明されていない。**推測：`kdump-tools`によるメモリの予約である**（40節）。**確認していない。** 確かめる場合は、カーネルのコマンドラインに与えられている`crashkernel`の指定を見る | 未確認のまま残っていること |
| **PHPのコードを打ち消しコミットで戻す経路が、一度も実行されていない**（30節）。理屈の上では競合せずに作れるが、**実際に戻した実績が無い。** 確認するには壊れたデプロイを1回行う必要があるため、現時点では行わないと決めた | 未確認のまま残っていること |

この節は複数の種別（対処しないと決めたこと、観測できない設定、条件付きの再検討）を含む。項目数が少ないため見出しを分けず、種別の列で区別している。

**`Permissions-Policy`：検討したうえで入れない。**

判断（入れない）は 39節・判断4 から変わらない。7-9 で CSP と一緒に判断するとしていた位置づけを、ここで「入れない」に確定する。

- このサイトには、注入されたスクリプトが機能 API を呼ぶ経路が実質的に存在しない。コンテンツを書くのは本人のみ、ユーザー投稿なし、問い合わせフォームの入力は表示に回らない。残る経路は依存パッケージの汚染だが、そこで実際に被害となるのは送信先の書き換えや偽フォームの差し込みであり、それを止めるのは CSP である。
- カメラ・マイク・位置情報は、このヘッダが無くてもブラウザが許可プロンプトを出す（推測：一般的なブラウザ挙動としてそう理解している。一次出典は当てていない）。無言で使われることはない。
- 別オリジンの iframe には、親が `allow` 属性で明示的に渡さない限りこれらの機能は渡らない（推測：同上）。
- 一方、入れると継続的な負担が生まれる。新しい機能 API が追加されても既存のヘッダはそれを拒否せず、カバー範囲が黙って減っていく（39節・判断4：書き漏らしたものは有効なまま残る）。この劣化を知らせる仕組みは無い。
- 経緯：この判断の理由は2回誤っていた。1回目「自分のコードがカメラを呼ばない」（自分のコードと注入されたコードを混同していた）、2回目「CSP と効く場面が同じ」（層が違う）。判断そのものは3回とも変わっていない。

**再検討すべき条件**（上の根拠4つに、それぞれ対応させる）

| 根拠 | 崩れる条件 |
| --- | --- |
| 注入されたスクリプトが機能APIを呼ぶ経路が実質的に存在しない（本人以外がコンテンツを書く・ユーザー投稿が入る・問い合わせフォームの入力が表示に回る、のいずれかに該当しない） | 本人以外がコンテンツを書くようになる、ユーザー投稿が入る、または問い合わせフォームの入力が表示に回るようになったとき |
| カメラ・マイク・位置情報は、このヘッダが無くてもブラウザが許可プロンプトを出す（推測、一次出典は未確認） | この推測が誤りだと判明したとき |
| 別オリジンのiframeには、親が`allow`属性で明示的に渡さない限り機能が渡らない（推測、未確認） | この推測が誤りだと判明したとき |
| 入れると継続的な負担が生まれる（新しい機能APIが追加されても既存のヘッダは拒否せず、カバー範囲が黙って減っていく） | カバー範囲が黙って減っていくことを知らせる仕組みができたとき |

CSP は引き続き 7-9 で判断する（41-5 を参照）。

---

### 41-8. 判断は済んでいるが、理由の記録が無いもの

**`open_basedir`：検討したうえで見送った。見送った理由が記録されていない。**

7-3c より前の判断であり、この文書にも統一ログにも理由が残っていない。**理由の記録が無いという事実そのものを、ここに記録しておく。**

再検討する場面（PHP の設定を触るカード、またはセキュリティの見直し）で、理由を作り直すか、判断そのものをやり直す。

**`.npmrc` の `ignore-scripts=true`：初期セットアップから入っている。入れた理由の記録がない。**

`.npmrc`（Git 管理下）に `ignore-scripts=true` があり、依存パッケージのインストール時スクリプト（`preinstall` / `install` / `postinstall`）が実行されない状態になっている。3-2 の初期セットアップのコミットに、単独ではなく一括で含まれて入った。この文書にも統一ログにも入れた理由が残っておらず、開発者本人にも設定した記憶がない。**理由の記録が無いという事実そのものを、ここに記録しておく。**

npm はカレントディレクトリからプロジェクトの `.npmrc` を探して適用する。`-g`（グローバルインストール）でも同じで、リポジトリの中から実行した `npm install -g` にも効く。このため、開発機での作業と CI の `npm ci`（`ci.yml`）の2か所に適用されている。実際、リポジトリ内から実行した `npm install -g` が postinstall を実行できず、開発機で Claude Code が起動しなくなる形で表面化した（7-4d の作業中）。7-4e より前は本番サーバーの `bin/deploy.sh` の `npm ci` にも効いていたが、7-4e でビルドが CI へ移り、サーバーでは `npm` を実行しなくなった（21節・30節）。

- 効果：依存パッケージのインストール時スクリプトが実行されない。悪意あるパッケージが最もよく使う経路が塞がれる。ただし `npm run build` の中で動くコード（ビルドツールのプラグインや設定ファイル）は通常のモジュール読み込みなので止まらない。
- 代償：postinstall を必要とするパッケージを静かに壊す。エラーが分かりにくい形で出る。上記の Claude Code のインストール障害がその実例。

判断すべきこと：意図して維持するのか、外すのか。維持するなら理由を記録する。21節（`npm audit` のゲート）はこの設定を根拠として書き足せる位置にあるが、理由が定まるまでは書き足さない（根拠にした設定が後で消えると、根拠が黙って崩れるため）。

（`.npmrc` にはもう1行 `audit=true` がある。npm の既定と同じ挙動を明示しているだけの別設定で、認証情報は含まれていない。）
...
| 項目 | 解決 |
| --- | --- |
| ESM（Ubuntu Pro）の有効化 | 7-3c-3 で有効化しないと判断（32節） |
| `pm.max_children` の調整 | 7-3c-3 で実測のうえ 10 に変更（26節・27節） |
| `opcache.max_accelerated_files` の実測 | 7-3c-3 の実測で既定値のままでよいと確認（28節） |
| `bootstrap/cache` のグループ書き込み権限 | 7-3c-3 で締めた（29節） |
| `default` がパッケージ管理下でありながら書き換えられている | 25節末尾で元に戻した |
| `default.bak` の要否 | 削除した（25節） |
| 1節で前提としていた、サーバー上に復元すべき固有データが存在しないという状態が成り立たなくなった | 1節で解決（判断は維持、理由を更新） |
| `/.well-known/acme-challenge/` の置き場所が未確定（server の root を継承しており、certbot が書くとリポジトリが汚れる） | 35節。リポジトリ外の専用ディレクトリ `/var/www/certbot` に置いた |
| 443側の受け皿（catch-all）が未設定 | 36節（`ssl_reject_handshake`） |
| `live/` のリンクが更新時に張り替わる挙動が未確認 | 37節。`archive/` に `cert2.pem` が増えることを確認した |
| 証明書の更新が一度も走っていない | 37節。本番で1回実行し、張り替わることを確認した |
| certbot の ARI 対応が未確認 | 更新設定ファイルで確認した |
| Let's Encrypt は期限切れの警告メールを送らないため、更新失敗の検知手段を自前で用意する必要がある | 37節。サーバーが提示する証明書を見る検知手段を用意した |
| 暗号化された経路であることが PHP に伝わっているか未確認 | 38節（実測。対応不要だった） |
| `APP_URL` が `http://<SERVER_IP>` のまま | 38節 |
| 33節の平文で流れるという記述が解消後も残っている | 33節に追記して解決 |
| `config:cache` が権限を緩めるという推測 | 実測になった（38節・30節） |
| HTTP/2 が未設定 | 39節 |
| `ssl_session_cache` が未設定 | 39節 |
| `gzip_types` を未確認 | 39節 |
| `robots.txt` が圧縮によって 74→89 バイトに増える | 39節・判断6 に記載済み（対処しない判断とその理由を含む） |
| `ssl_session_cache` が再利用の原因かどうか未確認 | 39節・判断3 に記載済み（未確認である旨を含む） |
| 手順書の判断の根拠が検証されていない（Issue #28：根拠ラベルの無い主張6件、および他節の記述を鉤括弧で引用している箇所16件） | 7-4a-1（根拠の監査）で監査・修正した。6件は出典の追加・書き分けで解消、引用16件は鉤括弧を外して節番号参照に統一（うち3件は引用先に字句が無く節番号も誤っていた） |
| 手順書の実行順序が通しで検証されていない（Issue #27） | 7-4a-2 で「検証しない」と判断し、冒頭にその理由を記載した |
| 節番号での相互参照を名前での参照に変えるべきか | 7-4a-2 で「変えない」と判断。名前が一意でない節が複数あり（所有権・権限の4節、更新の4節）、名前参照にするには先に見出しの改名が要る。節番号は機械的に照合できる |
...
| `opcache.validate_timestamps = 0` の再検討（41-2） | 7-4f で実測のうえ、既定の `1` のまま維持すると判断した（16節）。再検討すべき条件も16節に記録した |
...
| dbusの再起動が先送りされたまま残っていた件（41-7にあったもの。40節を参照） | 実測（2026-09-23）：カーネル更新に伴う再起動で解消した。**このとき先送りされていたのは3件（dbus・networkd-dispatcher・unattended-upgrades）で、再起動で3件とも解消した。** 40節が定めた、`apt upgrade`の出力を最後まで読んで先送りを41-7に記録する手順は、2026-09-21と2026-09-23の2回とも機能した |
