...
| Resend | 34・41-3 | 現時点ではメール送信に使用しておらず、DNSレコードのみ存在する（34節）。未解決事項は41-3に記載 |
...
- ライブラリ（libcurlなど）が更新されると、needrestartが影響を受けるサービスの再起動を扱う。**扱い方は2通りあり、どちらになるかは出力を見ないと分からない。**
  - **自動で再起動する。** php8.5-fpmやnginxがこれにあたる。サービスの起動時刻が変わっていたら、この可能性を疑う
  - **再起動を先送りする。** 実測（2026-09-21）：`apt upgrade`の出力に`Service restarts being deferred:`の行が現れ、dbusの再起動が先送りされた。**このとき`/var/run/reboot-required`は立たない**（同日、該当なしを実測）。needrestartによる自動再起動も、9節が定める自動再起動も発動しないため、**次に何らかの理由でサーバーを再起動するまで、先送りされたサービスは古い状態で動き続ける。気づく経路は無い**

  推測：先送りされる理由は出力に現れない。dbusのように多くのサービスが依存する土台では、自動再起動の影響が大きいためだと考えられる。確認していない。

  **したがって、`apt upgrade`の出力は最後まで読むこと。**`Service restarts being deferred:`の行が現れた場合、先送りされたサービスを41-7に記録し、次に再起動する機会を意識すること。
...
| `umask 027` のもとで composer が新しいファイルを作る場面が未検証（30節）。**次に依存が変わるデプロイで確認する** | 条件付きの再検討 |
| dbusの再起動が先送りされたまま残っている（実測 2026-09-21。40節を参照）。`/var/run/reboot-required`が立たないため、自動では解消しない。**次にサーバーを再起動した時点で解消する。** 今回再起動しなかったのは、影響が小さいと推測していること（更新対象にdbus自身は含まれておらず、triggerの処理のみだった）と、作業中のカードの本題と無関係な変更を挟まないためである。**この推測は確認していない** | 対処しないと決めたこと |
