...
| 節 | 古くなる理由 |
|---|---|
| 13・18・21 | PHP・Composer・Node のバージョン。導入した時点のものが書いてある |
...
Node.js の実際の削除（apt パッケージの除去）を行った。新方式（7-4e）が本番で成功したことを確認したうえで、2段階に分けて進めた。取り消しにくい操作のため、各段階の実行前に `--dry-run` で削除対象を一覧確認した。
...
この一覧は `bin/deploy.sh` と同じ内容を2箇所に持っている。スクリプトを変更したときは、この一覧も直す必要がある（40節の維持依存の索引に記載）。中身は `bin/deploy.sh` を参照。
...
ある節が達成した状態が、別の節の手順を実行し続けなければ保たれない関係がある。実行が止まらないため、失敗しても気づきにくい。以下はその一覧であり、詳細は各節にある。
...
| 30節 → bin/deploy.sh | `bin/deploy.sh` に段階を足したり順序を変えたりしたとき。30節の「`bin/deploy.sh` が行うこと（順に）」の一覧が古くなる | 無い。人が両方を直す必要がある。一覧を消せば消滅するが、bash を読まずに何が起きるか分かる価値を優先し、承知のうえで残している |
| 30節・21節 → ci.yml と deploy.yml の build ジョブ（7-4e） | 本番に載るビルドは `deploy.yml` の `build` ジョブが作る。`ci.yml` が検証するのは `ci.yml` 自身の build ステップ。両者の `npm` コマンドや `node-version` がずれると、CI が緑でも本番に載る成果物が検証したものと違いうる | 片方を変えたらもう片方も合わせる。`package.json` の `engines` とも揃える |
...
`21節 → （回復手順なし）`（Node と npm の版が上がると、ビルド成果物が環境をまたいで一致するという実測が成り立たなくなる）の1本は、7-4e で解消した。ビルドを行う場所が CI の1か所になったため、「環境をまたいで一致する」という要件そのものが無くなった（自動化ではなく消滅）。かわりに 7-4e で `ci.yml` と `deploy.yml` の build を揃える依存が1本増えた（上の表）。
...
| `deploy.yml` に応答確認ステップを足す | Basic 認証を撤去したら、デプロイ後に公開 URL を引いて表示できることを確認するステップを `deploy.yml` に追加する。引くパスは `ci.yml` の本番構成チェックと同じ6本を想定。30節の「サイトの応答確認を含めない」判断は、この時点で解除される（7-4d で追加） |
