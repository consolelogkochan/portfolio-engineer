...
| `umask 027` のもとで composer が新しいファイルを作る場面が未検証（30節）。**次に依存が変わるデプロイで確認する。** **【注記（7-5b）】この検証の機会は、当面来ない見込みである。** 7-5bで送信方式をSMTPにしたため、**パッケージの追加が発生しなかった。次にcomposerで依存を増やすときまで、この項目は検証されないまま残る。** | 条件付きの再検討 |
