...
Node.js の実際の削除（apt パッケージの除去）は、新方式が本番で成功したことを確認してから別手順で行う。それまでは apt の Node が残る。
...
- **Node.js をサーバーから削除すれば、universe 依存の大半が消える。** 7-4e でビルドを CI へ移し、サーバーで Node を使う理由は無くなった（21節・30節）。apt パッケージ（`nodejs` / `npm`）の除去は、新方式が本番で成功したことを確認してから別手順で行う。ここに挙げた 3 件はいずれも `apt install npm` が引き込んだもので、除去すれば消える見込み
