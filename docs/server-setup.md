...
### 41-1. 7-4 の先頭（手順書の監査）で回収する

**7-4 の最初のカードは、この文書そのものの監査とする。** 以下はいずれも、手順書の書き方・検証状態に関する項目である。

| 項目 | 補足 |
| --- | --- |
| 手順書の判断の根拠が検証されていない | Issue 登録済み |
| 手順書の実行順序が通しで検証されていない | Issue 登録済み |
...
出典：`umask 027` のもとで新規作成される権限は、ファイル 640・ディレクトリ 750 である（作成時に要求されるモード 666 / 777 から umask のビットを落とすため。`umask(2)`。https://man7.org/linux/man-pages/man2/umask.2.html ）。**グループ書き込みは付かない。**
...
| 項目 | 補足 |
| --- | --- |
| Resend のレコードが apex ベースで登録されている（`send` と `resend._domainkey`） | 引き継ぎ資料の `mail.ikshowcase.site` という記載と一致しない。どちらに寄せるかを決める |
...
| Basic 認証のユーザー名が OS のユーザー名（SSH の `AllowUsers` 対象）と同一 | 401 はユーザー名を明かさず、SSH はパスワード認証を無効化済みのため実害は小さいが、7-3a で「ありふれない名前を選ぶ」とした判断とは整合しない。**認証の撤去で解消する** |
...
| CSP と `Permissions-Policy` の設定 | 両者は同じ場面（スクリプトの注入）で効くため一緒に判断する。許可すべき読み込み先は、実コンテンツと画像が入った後でないと確定しない |
...
| 項目 | 種別 |
| --- | --- |
| 古い証明書と秘密鍵が `archive/` に残り、それぞれの期限まで有効なままである | 対処しないと決めたこと |
...
| 項目 | 解決 |
| --- | --- |
| ESM（Ubuntu Pro）の有効化 | 7-3c-3 で「有効化しない」と判断（32節） |
...
| 1節「サーバー上に復元すべき固有データが存在しない」が成り立たなくなった | 1節で解決（判断は維持、理由を更新） |
...
| 33節の「平文で流れる」という記述が解消後も残っている | 33節に追記して解決 |
